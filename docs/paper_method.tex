...
Further simplifications occur when including only the dominant harmonics $h^{\rm P}_{22}$, $h^{\rm P}_{2,-2}$ in the precessing-frame waveform (as is done for instance in PhenomP~\cite{Hannam+13}, as well as in our toy model~\ref{subsec:precmodel}), we have in this case for $f<0$ \SM{[all $f$ ?]}
\begin{subequations}
\begin{align}
...
\jgb{In this paper, we will need this model only as an example for demonstrating our frequency-domain precession response, but this or a similar approach may be also useful in borader gravitational wave analysis applications. In that context,}
...
%  \includegraphics[width=.32\linewidth]{plots/fom_--_A2.pdf}
  ...
  \caption{Error estimates of the approximation as defined in~\eqref{eq:deffom} for the precession modulations, for the three cases listed in Table~\ref{tab:precexamples} \SM{[complete caption]}}
  \label{fig:fomprec}
\end{figure*}
...
We now assess the accuracy of the transfer function computation, applying the formalism of Secs.~\ref{} and \ref{} to our three examples listed in Table~\ref{tab:precexamples} and comparing the result to a reference numerical computation.
...
\SM{[discuss the shortcoming of our LISA treatment for precessing signals]}
