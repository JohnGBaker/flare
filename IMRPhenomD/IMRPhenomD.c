...
static int IMRPhenomDGenerateh22FDAmpPhase_internal(
    ListmodesCAmpPhaseFrequencySeries** listhlm, /**< [out] FD waveform */
    gsl_vector* freq,                   /**< Input: frequencies (Hz) on which to evaluate h22 FD - will be copied in the output AmpPhaseFDWaveform. Frequencies exceeding max freq covered by PhenomD will be given 0 amplitude and phase. */
    const double tRef,                  /**< Time shift, 0 corresponds to peak approximatively set to time=0 (s) */
    const double phiRef,                /**< Orbital phase at fRef (rad) */
...
int IMRPhenomDGenerateh22FDAmpPhase(
  ...
  int status = IMRPhenomDGenerateh22FDAmpPhase_internal(listhlm, freq, tRef, phiRef, fRef, m1, m2, chi1, chi2, distance);
...
int IMRPhenomDGenerateh22FDAmpPhase_internal(
  ...
  /* The input pointer to the list is supposed to be NULL */
  if(*listhlm)
  {
  ...
  /* Initialize the complex series for h22 - imaginary part of the amplitude is not used */
  CAmpPhaseFrequencySeries *h22freqseries = NULL;
  CAmpPhaseFrequencySeries_Init(&h22freqseries, n);
  gsl_vector_set_zero(h22freqseries->amp_imag);
  ...
  //time shift so that peak amplitude is approximately at t=0
  //For details see https://www.lsc-group.phys.uwm.edu/ligovirgo/cbcnote/WaveformsReview/IMRPhenomDCodeReview/timPD_EDOMain
  const double twopit0M = DPhiMRD(pAmp->fmaxCalc, pPhi);
  const double twopitRefM = 2*PI*tRef/M_sec;
  ...
  double* freqwf = h22freqseries->freq->data;
  double* amp = h22freqseries->amp_real->data;
  double* phase = h22freqseries->phase->data;
  #pragma omp parallel for
  for (size_t i = 0; i < n; i++)
  {
    ...
    if(f[i]>fCut) { // for frequencies exceeding the maximal frequency covered by PhenomD, put 0 amplitude and phase
      ...
      else
      {
        ...
        phi = phi + (twopitRefM - twopit0M) * (Mf - MfRef) - phifRef + 2.*phiRef;
        amp[i] = amp0 * a;
        phase[i] = phi;
      }
    }
  ...
  *listhlm = ListmodesCAmpPhaseFrequencySeries_AddModeNoCopy(*listhlm, h22freqseries, 2, 2);
